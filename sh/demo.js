...
import pug_conf from "~/conf/web/pug.js";
...
const inline = (cdn_dir, css_path) => {
    if (!existsSync(css_path)) return "";
    const import_re = /@import\s+["']\.\/([^"']+)\.css["'];/g;
    return read(css_path).replace(import_re, (match, dep_name) => {
      let dep_path = join(cdn_dir, dep_name + ".css");
      if (!existsSync(dep_path)) {
        dep_path = join(cdn_dir, "_" + dep_name + ".css");
      }
      if (!existsSync(dep_path)) {
        dep_path = join(cdn_dir, dep_name + "_.css");
      }
      return inline(cdn_dir, dep_path);
    });
  },
  copy = (src, dest_dir) => {
    if (existsSync(src)) {
  ...
  compile = (dist_dir) => {
    ...
    if (existsSync(styl_dir)) {
      const files = readdirSync(styl_dir)
          ...
          .sort((a, b) => (a == "reset.styl" ? -1 : b == "reset.styl" ? 1 : a.localeCompare(b))),
        css_raw = files
          .map((file) => {
  ...
  serve = (port, dist_dir, cdn_dir) => {
    Bun.serve({
      ...
      fetch(req) {
        ...
        const cdn_file = join(cdn_dir, pathname);
        if (existsSync(cdn_file) && statSync(cdn_file).isFile()) {
          return new Response(Bun.file(cdn_file));
        }
  ...
  main = async () => {
    ...
    const absolute_path = com(raw_path),
    ...
    if (!existsSync(absolute_path) || !statSync(absolute_path).isDirectory()) {
      console.error("Invalid folder path: " + raw_path);
      process.exit(1);
    }
    ...
    if (!existsSync(htm_path)) {
      console.error("Demo HTML file not found. Make sure _.htm exists in " + demo_dir);
      process.exit(1);
    }
    ...
    const dist_dir = join(ROOT, "dist/demo", name),
      ...
      src_js = join(cdn_dir, name + ".js");

    let src_css = join(cdn_dir, name + ".css");
    ...
    const has_js = copy(src_js, dist_dir);
    let has_css = false;

    if (existsSync(src_css)) {
      const file_name = basename(src_css);
      write(join(dist_dir, file_name), inline(cdn_dir, src_css));
      const map = src_css + ".map";
      if (existsSync(map)) {
        cpSync(map, join(dist_dir, file_name + ".map"));
      }
      has_css = true;
    }

    copy(join(cdn_dir, "x.js"), dist_dir);
    ...
    if (has_js) {
      ...
      if (has_default) {
        ...
        let var_name = name;
        if (new RegExp("\\b" + camel + "\\b").test(demo_js)) {
          var_name = camel;
        }
        demo_js = "import " + var_name + ' from "./' + basename(src_js) + '";\n' + demo_js;
      } else {
        demo_js = 'import "./' + basename(src_js) + '";\n' + demo_js;
      }
    }

    const compile_fn = pug.compileFile(join(ROOT, "pug/demo.pug"), {
        basedir: ROOT,
      }),
      ...
      is_interactive =
        ...
        args.some((x) => ["-o", "--open", "-i", "--interactive"].includes(x));
