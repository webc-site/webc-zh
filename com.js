...
import { extract as extractSvgs } from "~/cli/src/compile/svg.js";
  ...
  metadata = (name) => {
    ...
    if (existsSync(readme_path) && existsSync(demo_path)) {
      ...
      if (existsSync(com_js)) {
        const js_content = read(com_js);
        if (js_content.includes("export default")) {
          const demo_js_path = join(com_path, "demo", "_.js"),
            demo_js = existsSync(demo_js_path) ? read(demo_js_path) : "",
            camel = name[0].toLowerCase() + name.slice(1),
            var_name = new RegExp("\\b" + camel + "\\b").test(demo_js) ? camel : name;
          cdn_files.push("\nimport " + var_name + ' from "$URL/' + name + '.js"\n');
        } else {
          cdn_files.push('import "$URL/' + name + '.js"');
        }
      }
      if (styl_files.length > 0) {
        cdn_files.push('<link href="$URL/' + name + '.css" rel="stylesheet">');
      }
